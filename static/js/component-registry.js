...
.toggle-btn:hover {
  ...
  'News': (props) => `<template x-if="published">
...
<body>
  ...
  <div>
    <h2>Loops</h2>
    <ul><template x-for="item in props.items">
        <li><span x-text="item"></span></li></template>
    </ul>
  </div>
</body>
</html><style>
...
@media only screen and (min-width: 64rem) {
  ...
  '../components/test_all_content.html': (props) => `<div class="all-content-test">
...
user: <span x-text="$store.auth.user ? $store.auth.user.name : 'null'" style="color: #10b981;"></span></pre>
          ...
          <div class="state-card store-card-hover" :style="\`background: $${$store.theme.mode === 'light' ? '#f9fafb' : '#1a202c'}; border-color: $${$store.theme.mode === 'light' ? '#e5e7eb' : '#4a5568'};\`">
            ...
            <div class="cart-items-list">
              ...
              <ul><template x-for="item in props.$cart.items">
...
@media only screen and (min-width: 768px) {
  ...
  '../content/news.html': (props) => `<template x-if="published">
<div class="blog-container main-content-wrapper">
  <div class="main-content">
    <article class="blog-article">
      <!-- Hero Image -->
...
<body>
	...
	<main>
		...
		<!-- allContent removed - components inherit from body x-data via Alpine scope -->
  ...
  'Jim-test': (props) => `<template x-for="component in props.components"><template x-if="component.name === 'demo_header'">
    <!-- Build time display -->
    <div style="background-color: #f0f0f0; padding: 0.5rem 1rem; border-radius: 0.25rem; margin-bottom: 1rem; font-size: 0.875rem; color: #666;">
      ⚡ Build time: <strong>${props.buildTime}</strong>
    </div>
    ...
    <!-- Greeting -->
    <h1><span x-text="component.fields.salutation"></span> <span x-text="component.fields.name"></span>!</h1>

    <!-- Basic conditionals --><template x-if="component.fields.name.length > 3">
      <div id="praise"><span x-text="component.fields.name"></span> is a long name</div><template x-if="component.fields.age > 1">
        <div>Has been born</div></template></template><template x-else-if="component.fields.name.length == 2">
      <div id="praise"><span x-text="component.fields.name"></span> is medium</div></template><template x-else>
      <div id="praise"><span x-text="component.fields.name"></span> is a short name</div></template></template><template x-else-if="component.name === 'component_demos'">
    <!-- Component usage with props --><template x-if="component.fields.age > 0">
      ...
      </div></template></template><template x-else-if="component.name === 'loop_demos'">
    ...
    <div class="animals" style="background-color: #f0f0f0; padding: 1rem; margin: 2rem 0;">
      <h2>Animals Loop with Advanced Features</h2><template x-for="animal in component.fields.animals"><template x-if="animal == \"cat\"">
          <div>Hi <span x-text="animal"></span>!</div></template><template x-else>
          <div>Bye <span x-text="animal"></span>.</div></template>
        <div :class="animal"><span x-text="component.fields.name"></span> likes: <span x-text="animal"></span>s</div>
    ...
    <div style="background-color: #f9fafb; border: 1px solid #e5e7eb; border-radius: 0.5rem; padding: 1.5rem; margin: 2rem 0;">
      ...
      <div style="background-color: white; padding: 1rem; border-radius: 0.375rem; margin-bottom: 1.5rem;"><template x-for="animal in props.["🦄 unicorn", ...component.fields.animals]">
      ...
      <div style="background-color: white; padding: 1rem; border-radius: 0.375rem;"><template x-for="word in props.["Waller", "loves Plenti", "uses AI", "is Australian"]">
          <div style="padding: 0.25rem 0; color: #1f2937;"><span x-text="component.fields.name"></span> <span x-text="word"></span></div></template><template x-for="phrase in props.["rocks", "codes", "innovates"]">
          <div style="padding: 0.25rem 0; color: #1f2937; font-style: italic;"><span x-text="component.fields.name"></span> <span x-text="phrase"></span>!</div></template>
      </div>
    </div></template><template x-else-if="component.name === 'interactive_demos'">
    <!-- Interactive Notification Examples -->
    <div style="margin: 2rem 0;">
      <h2>Interactive Notification Examples</h2>
      <div style="display: flex; gap: 0.5rem; margin-bottom: 1rem; flex-wrap: wrap;"><template x-for="notif in component.fields.notifications">
...
<div style="padding: 2rem; max-width: 800px; margin: 0 auto;">
  ...
  'Pages': (props) => `<!-- Pages Layout - Renders dynamic components from content JSON --><!-- NOTE: <head> provided by wrapper layout (includes Alpine.js + runtime-components.js) --><!-- NOTE: No fence section - uses variables from parent scope (html.html wrapper) --><template x-for="component in props.content.components"></template>`,
  'Featured_posts_sidebar': (props) => `<div class="blog-sidebar">
  <div class="blog-featured-group">
    <span class="blog-header">Featured Posts</span>

    <!-- Filter posts by type and display featured ones --><template x-for="post in Object.values(allContent).filter(c => c.type === 'news')"><template x-if="post.fields?.post?.id === '6' || post.fields?.post?.id === '5' || post.fields?.post?.id === '4'">
        <a :href="post.path" class="blog-feature">
          <picture class="blog-featureImage">
            <img :alt="post.fields.blogImage?.alt || 'Blog image'" decoding="async" :src="post.fields.blogImage?.src" :width="post.fields.blogImage?.width || 400" :height="post.fields.blogImage?.height || 300">
          </picture>

          <div class="content-group">
            <h3 class="feature-h3"><span x-text="post.fields.title"></span></h3>
...
<body>
  ...
  '../components/featured_posts_sidebar.html': (props) => `<div class="blog-sidebar">
  <div class="blog-featured-group">
    <span class="blog-header">Featured Posts</span>
...
<div class="container">
  ...
  <div class="section">
    ...
    <div class="card">
      ...
      <p>Login status: ${props.isLoggedIn}</p>
    </div>
  </div>
  ...
  <div class="section">
    ...
    <div class="card">
      <h3>User Authentication Status</h3><template x-if="isLoggedIn">
        <p>You are currently logged in.</p></template><template x-else>
        <p>You are not logged in.</p></template>
    </div>
  </div>

  <!-- 3. Simple Content -->
  <div class="section">
    <h2 class="section-title">3. Simple Template Demo</h2>
    <div class="card">
      <p>This is a simplified comprehensive test page.</p>
      <p>For complex JavaScript functions, use Alpine.js x-data directly in your HTML.</p>
    </div>
  </div>
</div>
...
.cart-total {
  ...
  '../content/jim-test.html': (props) => `<template x-for="component in props.components"><template x-if="component.name === 'demo_header'">
    <!-- Build time display -->
    <div style="background-color: #f0f0f0; padding: 0.5rem 1rem; border-radius: 0.25rem; margin-bottom: 1rem; font-size: 0.875rem; color: #666;">
      ⚡ Build time: <strong>${props.buildTime}</strong>
    </div>
  ...
  '../content/pages.html': (props) => `<!-- Pages Layout - Renders dynamic components from content JSON --><!-- NOTE: <head> provided by wrapper layout (includes Alpine.js + runtime-components.js) --><!-- NOTE: No fence section - uses variables from parent scope (html.html wrapper) --><template x-for="component in props.content.components"></template>`,
...
<body style="padding: 2rem;">
  ...
  'Test_all_content': (props) => `<div class="all-content-test">
    <h2>All Content Test</h2>
    <p>Total pages loaded: <span x-text="Object.keys(allContent || {}).length"></span></p>
...
.toggle-btn:hover {
  ...
  '../components/whyChoose2425.html': (props) => `<!-- ============================================ --><!--                 Why Choose                   --><!-- ============================================ --><section id="why-2425">
  <div class="cs-container">
