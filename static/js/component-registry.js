...
<body>
  ...
  '../components/LoginStatus.html': (props) => `<div class="login-status">
  <!-- Server renders initial state from JSON prop --><template x-if="initialLoggedIn">
    <!-- Initially logged in: hide when Alpine detects logged out -->
    <div class="logged-in" x-show="$store.auth.isLoggedIn">
      <span class="user-info">
        👤 Welcome, <strong x-text="$store.auth.user ? $store.auth.user.name : 'User'"></strong>
      </span>
      <button @click="$store.auth.logout()" class="btn btn-secondary">
        Logout
      </button>
    </div>
    <!-- Hidden initially, shows when Alpine detects logged in -->
    <div class="logged-out" x-show="!$store.auth.isLoggedIn" style="display: none;">
      <span class="guest-info">👋 Guest</span>
      <button @click="$store.auth.login()" class="btn btn-primary">
        Login
      </button>
    </div></template><template x-else>
    <!-- Initially logged out: hide when Alpine detects logged in -->
    <div class="logged-in" x-show="$store.auth.isLoggedIn" style="display: none;">
      <span class="user-info">
        👤 Welcome, <strong x-text="$store.auth.user ? $store.auth.user.name : 'User'"></strong>
      </span>
      <button @click="$store.auth.logout()" class="btn btn-secondary">
    ...
    <!-- Shows initially, hides when Alpine detects logged in -->
    <div class="logged-out" x-show="!$store.auth.isLoggedIn">
      <span class="guest-info">👋 Guest</span>
      <button @click="$store.auth.login()" class="btn btn-primary">
        Login
      </button>
...
<body>
  ...
  'LoginStatus': (props) => `<div class="login-status">
  <!-- Server renders initial state from JSON prop --><template x-if="initialLoggedIn">
    <!-- Initially logged in: hide when Alpine detects logged out -->
    <div class="logged-in" x-show="$store.auth.isLoggedIn">
      <span class="user-info">
