...
<div>Hello</div>`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			markup, script, style := RenderWithStores(template, transformed, storeDefinitions, "test.html", "", nil)
...
<body>
  <div>{$auth.isLoggedIn}</div>
	...
	// Render
	markup, script, _ := RenderWithStores(parsedTemplate, transformed, storeDefinitions, "test.html", "", nil)
...
func TestEmptyStoresNoScript(t *testing.T) {
	...
	_, script, _ := RenderWithStores(parsedTemplate, transformed, map[string]string{}, "test.html", "", nil)
